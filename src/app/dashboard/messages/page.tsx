...
function lastMessage(c: Conversation): Message | undefined {
...
export default function MessagesPage() {
  ...
  const [activeId, setActiveId] = useState<string>(
    seedConversations[0]?.clientId ?? "",
  );
  const [draft, setDraft] = useState("");

  const active = threads.find((t) => t.clientId === activeId);
  const activeClient = active ? getClient(active.clientId) : undefined;
  ...
  function selectThread(id: string) {
    ...
    setThreads((prev) =>
      prev.map((t) => (t.clientId === id ? { ...t, unread: 0 } : t)),
    );
  }
